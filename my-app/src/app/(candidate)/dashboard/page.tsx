...
import { Briefcase, Clock, CheckCircle2, XCircle, Eye } from "lucide-react";
...
export default function CandidateDashboard() {
  ...
  const myApplications = applications.filter(
    (app) => app.candidateEmail === profile.email,
  );
    ...
    <div className="space-y-8 animate-fade-in font-sans">
      <div>
        <h1 className="text-2xl md:text-[28px] font-bold text-gray-900 tracking-tight flex items-center gap-2">
          Welcome back, {profile.name.split(" ")[0] || "there"}! <span className="text-2xl">👋</span>
        </h1>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-5">
        <div className="bg-[#f4f7ff] p-6 rounded-[20px] flex flex-col gap-4">
        ...
        <div className="bg-[#fffdf4] p-6 rounded-[20px] flex flex-col gap-4">
        ...
        <div className="bg-[#f4fff8] p-6 rounded-[20px] flex flex-col gap-4">
          <div className="w-10 h-10 bg-white rounded-xl shadow-sm flex items-center justify-center text-emerald-500">
            <Eye className="w-5 h-5" />
          </div>
          <div>
            <p className="text-[32px] font-bold text-gray-900 leading-none">{activeCount}</p>
            <p className="text-[13px] text-gray-500 font-semibold mt-2">Active</p>
          </div>
        </div>

        <div className="bg-[#fff4f4] p-6 rounded-[20px] flex flex-col gap-4">
      ...
      <div className="space-y-6">
                ...
                <tbody className="divide-y divide-gray-50">
                  {myApplications.slice(0, 6).map((app) => (
                    ...
                    >
                      ...
                      <td className="py-4 px-6 text-gray-500 font-medium">{app.appliedDate}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <div className="bg-[#f8fafc] rounded-2xl border border-blue-100/50 p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          ...
          <Link
            href="/jobs"
            onClick={() => setCandidatePage("jobs")}
            className="px-5 py-2.5 bg-white border border-gray-200 text-blue-600 font-semibold text-sm rounded-xl hover:bg-slate-50 transition-colors whitespace-nowrap shadow-sm"
          >
            View Jobs
          </Link>
        </div>

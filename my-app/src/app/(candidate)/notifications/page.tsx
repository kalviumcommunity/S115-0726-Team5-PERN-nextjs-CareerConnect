...
type FilterType = 'All' | 'Unread' | 'Status Updates' | 'New Applications';
...
export default function NotificationsPage() {
  ...
  useEffect(() => {
    // Keep existing behavior of marking notifications as read when page mounts
    markNotificationsAsRead();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  const filters: FilterType[] = ['All', 'Unread', 'Status Updates', 'New Applications'];

  const filteredNotifications = (notifications || []).filter((notif: any) => {
    if (activeFilter === 'All') return true;
    if (activeFilter === 'Unread') return !notif.read;
    if (activeFilter === 'Status Updates') return ['viewed', 'accepted', 'rejected'].includes(notif.type);
    if (activeFilter === 'New Applications') return notif.type === 'new_application';
    return true;
  });
  ...
  const getIcon = (type: string) => {
    switch (type) {
      case 'viewed': return <Eye className="w-5 h-5 text-blue-600" />;
      case 'accepted': return <CheckCircle2 className="w-5 h-5 text-green-600" />;
      case 'rejected': return <XCircle className="w-5 h-5 text-red-600" />;
      case 'new_application': return <Info className="w-5 h-5 text-indigo-600" />;
  ...
  const getIconBg = (type: string) => {
    switch (type) {
      ...
      case 'new_application': return 'bg-indigo-100';
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8">
        <div>
          <div className="flex items-center gap-3 mb-2">
            ...
            {unreadCount > 0 && (
              <span className="bg-blue-100 text-blue-700 text-xs font-bold px-2.5 py-0.5 rounded-full animate-count-pop">
                {unreadCount} new
              </span>
            )}
          </div>
          <p className="text-gray-500">Stay updated on your job applications and profile views.</p>
        </div>
      </div>
            ...
            return (
              <div 
                ...
                className={`animate-slide-in-up ${staggerClass} relative flex items-start gap-4 p-5 rounded-2xl bg-white border transition-all duration-200 hover:shadow-md ${
                  !notification.read ? 'border-l-4 border-l-blue-500 border-y-blue-50 border-r-blue-50 bg-blue-50/30' : 'border-gray-100'
                }`}
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex justify-between items-start gap-2 mb-1">
                    ...
                    {!notification.read && (
                      <span className="w-2.5 h-2.5 bg-blue-600 rounded-full flex-shrink-0 mt-1.5 shadow-sm shadow-blue-200"></span>
                    )}
                  </div>
          ...
          <div className="flex flex-col items-center justify-center py-16 px-4 bg-white rounded-3xl border border-gray-100 shadow-sm animate-scale-in">
            ...
            <p className="text-gray-500 text-center max-w-sm">
              {activeFilter === 'All' 
                ...
                : `You don't have any ${activeFilter.toLowerCase()} notifications right now.`}
            </p>
          </div>
        )}
      </div>
